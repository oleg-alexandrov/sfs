...
#if [ "$win" = "7" ]; then pwin=$win7; fi

sm=0.06
level=2

dir=${sfs}_run$run # 1m_abc_sm${sm}_level${level}_run$run
#dir=${sfs}_1m_abc_sm${sm}_level${level}_run$run
mkdir -p $dir
gdal_translate -projwin $win ${st}/run-1m-DEM.tif $dir/init-DEM.tif
