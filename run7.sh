...
point2dem -r moon --csv-format '1:lon 2:lat 3:radius_km' --tr 3 --stereographic --proj-lon -88.572434 --proj-lat -85.832098 run7_subpix3/run-trans_source.csv -o run7_subpix3/run-trans_source-tr3
...
#crop goes from crop2 to crop12, with crop10 crop11 and crop12 having shadows
crop=crop9; lola7=run7_subpix3/run-trans_source-DEM.tif; di $lola7 run7_subpix3/run-$crop-DEM.tif; for f in $(ls -d sfs_adk*$crop*); do h=$(ls -altrd $f/*DEM*tif | grep -i -v hill | tail -n 1 | pc 0); echo $h; di $lola7 $h; done
...
if [ "$id" = "4" ]; then fa=0; wt=16000; ref=0; fi
...
if [ "$scale" = "1.2" ]; then thresh="0.00142167 0.000921485 0.0012485"; fi
if [ "$scale" = "1.0" ]; then thresh="0.00118473 0.000767904 0.00104041"; fi
if [ "$scale" = "0.9" ]; then thresh="0.00106625 0.000691114 0.000936373"; fi
if [ "$scale" = "0.8" ]; then thresh="0.00094778 0.000614323 0.000832332"; fi
if [ "$scale" = "0.7" ]; then thresh="0.000829308 0.000537533 0.00072829"; fi
if [ "$scale" = "0.6" ]; then thresh="0.000710835 0.000460742 0.000624249"; fi

thopt="";
if [ "$scale" != "" ]; then thopt="--shadow-thresholds '$thresh'"; fi

if [ "$fa" -ne "0" ]; then opt="--float-albedo"; else opt=""; fi


tid=$(echo $tile | perl -pi -e "s#^.*?tile-(.*?).tif#tile\$1#g")
echo tid=$tid

dir=sfs_adk_${tid}_reg${reg}_ref${ref}_wt${wt}_fa${fa}
...
sfs -i $tile $a.cub $d.cub $k.cub -o $dir/run --threads ${th} --smoothness-weight ${reg} --max-iterations 20 --reflectance-type $ref --use-approx-camera-models --coarse-levels 0 --crop-input-images --bundle-adjust-prefix run7_ba_manual/run --initial-dem-constraint-weight ${wt}  --use-rpc-approximation --float-exposure --float-cameras $thopt $opt > output_${dir}.txt 2>&1
#sfs -i run7_subpix3/run-${crop}-DEM.tif $a.cub $d.cub $k.cub -o $dir/run --threads ${th} --smoothness-weight ${reg} --max-iterations 50 --reflectance-type $ref --use-approx-camera-models --coarse-levels 0 --crop-input-images --bundle-adjust-prefix run7_ba_manual/run --initial-dem-constraint-weight ${wt}  --use-rpc-approximation --float-exposure --float-cameras $thopt $opt > output_${dir}.txt 2>&1
