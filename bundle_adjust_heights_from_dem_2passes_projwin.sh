...
echo Note that we filter input ip by proj win from dem

proj_win=$(~/bin/gdal_win.sh $dem)
proj_str=$(gdalinfo -proj4 lola/lola_clip_Site20_psr.tif |grep proj= | perl -pi -e "s#\'##g")
echo proj_win $proj_win
echo proj_str $proj_str
